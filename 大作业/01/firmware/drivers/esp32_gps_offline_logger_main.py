...
LED state for power-bank offline GPS collection:
...
- Power-on: solid ON for START_LED_ON_S seconds.
- Ready: three short flashes, then GPS capture starts.
- No GPS serial data: OFF, with one very short heartbeat every 3 seconds.
- NMEA received but no valid fix yet: slow 1 Hz blink.
- Valid GPS fix and data is being saved: two short flashes every 2 seconds.
- Error: fast blink.
...
def update_status_led(led, mode, now, t0):
    ...
    phase = ticks_diff(now, t0) % 3000
    if mode <= 0:
        # Mostly dark: alive heartbeat only.
        led_write(led, phase < 80)
    elif mode == 1:
        # Slow blink: NMEA is being saved, but no valid fix was seen recently.
        led_write(led, ticks_diff(now, t0) % 1000 < 500)
    else:
        # Double flash every 2 seconds: valid fix and data is being saved.
        phase = ticks_diff(now, t0) % 2000
        led_write(led, phase < 120 or 260 <= phase < 380)
...
def main():
    ...
    print("LED: no_nmea=short heartbeat, nmea_no_fix=slow blink, fix_and_save=double flash")
